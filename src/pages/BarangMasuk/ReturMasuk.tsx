...
import { getTransaksiMasukById, createRetur } from '../../service/barangMasuk';
...
export default function ReturMasuk() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    async function fetchDetail() {
      try {
        const tid = Number(id);
        const t = await getTransaksiMasukById(tid);
        if (!t) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await createRetur(transaksi.id, payload);
      if (!data || !data.status) {
  ...
  return (
    <>
      ...
      <PageMeta title="Retur Barang Masuk" description="Form retur barang masuk" />
      <PageBreadcrumb
        pageTitle="Retur Barang Masuk"
      ...
      <div className="w-full mt-8">
        <ComponentCard title={`Retur Transaksi #${transaksi.id}`}>
          <div className="space-y-6">
              ...
              <div>
                <span className="block text-sm text-gray-500 dark:text-gray-400">Supplier</span>
                <span className="font-semibold block text-gray-800 dark:text-white/90">{transaksi.supplier?.nama || '-'}</span>
              </div>
              <div>
