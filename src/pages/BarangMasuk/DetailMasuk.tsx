...
export default function DetailMasuk() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    async function fetchDetail() {
      try {
        const tid = Number(id);
        const data = await getTransaksiMasukById(tid);
        if (mounted && data) {
  ...
  return (
    <>
      <PageMeta
        ...
        description="Preview detail transaksi barang masuk"
      />
      <PageBreadcrumb
      ...
      <div className="w-full mt-8">
        <ComponentCard title="Detail Transaksi Barang Masuk">
