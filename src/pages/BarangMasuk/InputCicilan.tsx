...
import { getTransaksiMasukById, updateBerjangkaMasuk, UpdateBerjangkaPayload } from '../../service/barangMasuk';
...
export default function InputCicilan() {
  ...
  useEffect(() => {
    const fetchTransaksi = async () => {
      if (!id) return;
      const result = await getTransaksiMasukById(parseInt(id));
      if (result) {
        setTransaksi(result);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateBerjangkaMasuk(parseInt(id), payload);
  ...
  return (
    <>
      <PageMeta
        title="Input Cicilan"
        description="Input cicilan pembayaran transaksi"
      />
      <PageBreadcrumb
        pageTitle="Input Cicilan"
