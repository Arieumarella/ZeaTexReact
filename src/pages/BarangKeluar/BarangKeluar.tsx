...
export default function BarangKeluar() {
  ...
  const openNotaModal = async (item: any) => {
    ...
    try {
      const fullTx = await getTransaksiKeluarById(item.id);
      if (fullTx) {
  ...
  const handleUploadNota = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        id_pelanggan: selectedTransaksi.pelanggan?.id || null,
      ...
      const res = await updateTransaksiKeluar(selectedTransaksi.id, payload, selectedFile, false);
      if (res && res.status) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const res = await updateTransaksiKeluar(selectedTransaksi.id, payload, null, true);
          if (res && res.status) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (customer) params.customerId = Number(customer);
      ...
      const res = await getTransaksiKeluar(params);
      if (res && res.data) {
        const rows: any[] = [];
        let rowNum = 1;
      ...
      if (res && res.data) {
        ...
        (res.data || []).forEach((item: any, idx: number) => {
          ...
          rows.push({
            ...
            "Id Transaksi": item.id,
            "Tanggal Transaksi": formatDateWithMonth(item.tgl_transaksi),
            "Nama Pembeli": item.pelanggan?.nama || '',
        ...
        XLSX.writeFile(wb, `export-pajak-barang-keluar${fileRange}.xlsx`);
  ...
  const [customer, setCustomer] = useState(() => sessionStorage.getItem("barang_keluar_customer") || "");
  const [customers, setCustomers] = useState<any[]>([]);
  const [customerQuery, setCustomerQuery] = useState("");
  const [showCustomerDropdown, setShowCustomerDropdown] = useState(false);
  const [selectedCustomerName, setSelectedCustomerName] = useState(() => sessionStorage.getItem("barang_keluar_customerName") || "");
  const [tanggalStart, setTanggalStart] = useState(() => sessionStorage.getItem("barang_keluar_tglAwal") || "");
  const [tanggalEnd, setTanggalEnd] = useState(() => sessionStorage.getItem("barang_keluar_tglAkhir") || "");
  const [searchKdBarang, setSearchKdBarang] = useState(() => sessionStorage.getItem("barang_keluar_kdBarang") || "");
  const [page, setPage] = useState(() => {
    const savedPage = sessionStorage.getItem("barang_keluar_page");
    if (savedPage) {
  ...
  useEffect(() => {
    sessionStorage.setItem("barang_keluar_page", String(page));
  }, [page]);

  // Persist filter di sessionStorage agar tetap ada saat kembali dari halaman lain
  useEffect(() => {
    if (customer) sessionStorage.setItem("barang_keluar_customer", customer);
    else sessionStorage.removeItem("barang_keluar_customer");
    if (selectedCustomerName) sessionStorage.setItem("barang_keluar_customerName", selectedCustomerName);
    else sessionStorage.removeItem("barang_keluar_customerName");
    if (tanggalStart) sessionStorage.setItem("barang_keluar_tglAwal", tanggalStart);
    else sessionStorage.removeItem("barang_keluar_tglAwal");
    if (tanggalEnd) sessionStorage.setItem("barang_keluar_tglAkhir", tanggalEnd);
    else sessionStorage.removeItem("barang_keluar_tglAkhir");
    if (searchKdBarang) sessionStorage.setItem("barang_keluar_kdBarang", searchKdBarang);
    else sessionStorage.removeItem("barang_keluar_kdBarang");
  }, [customer, selectedCustomerName, tanggalStart, tanggalEnd, searchKdBarang]);
  ...
  }, [page, customer, tanggalStart, tanggalEnd, searchKdBarang]);
              ...
              {loadingNotaData ? (
                ...
                <>
                  ...
                  {selectedTransaksi && (
                    <div className="grid grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-850 rounded-xl p-4 text-xs border border-gray-100 dark:border-gray-800">
                      <div>
                        ...
                        <span className="font-semibold text-gray-850 dark:text-white">{selectedTransaksi.pelanggan?.nama || "-"}</span>
                      </div>
                      <div>
                  ...
                  {selectedNota ? (
                    ...
                    <div className="bg-gray-50 dark:bg-gray-955/40 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl py-8 flex flex-col items-center justify-center text-center">
