...
export default function EditManajemenUser() {
  ...
  const [idToko, setIdToko] = useState<number>(1);
  ...
  useEffect(() => {
    ...
    getUserDetail(Number(id)).then((res) => {
      if (res && res.status && res.user) {
        ...
        setIdToko(res.user.id_toko || 1);
        setRole(res.user.role || 'ADMIN_TOKO');
      }
      setLoading(false);
    });
  }, [id]);
        ...
        <ComponentCard title={`Form Edit User #${id || ''}`}>
          {loading ? (
            ...
            <form className="space-y-6" onSubmit={async (e) => {
              ...
              <div>
                <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-white">Password (Isi jika ingin ubah)</label>
